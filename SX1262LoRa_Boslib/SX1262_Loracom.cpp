...
void SX1262_Loracom::sx1262InitialSetup(){
  ...
  // Set standby mode
  loraSpi.spiBuff[0] = 0x80;
  loraSpi.spiBuff[1] = 0x00; 
  loraSpi.transferSPI(2);

// Clear device errors caused by using extern TCXO
  loraSpi.spiBuff[0] = 0x07;          
  loraSpi.spiBuff[1] = 0x00;          
  loraSpi.spiBuff[2] = 0x00;          
  loraSpi.transferSPI(3);
  
  // Set packet type to Lora
  loraSpi.spiBuff[0] = 0x8A;
  loraSpi.spiBuff[1] = 0x01; //Lora
  loraSpi.transferSPI(2);

 // Set DIO3 to controll osc
  loraSpi.spiBuff[0] = 0x97;
  loraSpi.spiBuff[1] = TCXO_VOLTAGE;
  loraSpi.spiBuff[2] = 0x00;
  loraSpi.spiBuff[3] = 0x0F;
  loraSpi.spiBuff[4] = 0xFF;
  loraSpi.transferSPI(5);

  // Call for calibratiom since extern TCXO
  loraSpi.spiBuff[0] = 0x98;
  loraSpi.spiBuff[1] = 0xD7;
  loraSpi.spiBuff[2] = 0xDB;
  loraSpi.transferSPI(3);

  // Set standby mode
  loraSpi.spiBuff[0] = 0x80;
  loraSpi.spiBuff[1] = 0x01; 
  loraSpi.transferSPI(2);
  ...
  loraSpi.spiBuff[0] = 0x86; 
  loraSpi.spiBuff[1] = (pllFrequency >> 24) & 0xFF;  //MSB of pll frequency
  loraSpi.spiBuff[2] = (pllFrequency >> 16) & 0xFF;  //
  loraSpi.spiBuff[3] = (pllFrequency >>  8) & 0xFF;  //
  loraSpi.spiBuff[4] = (pllFrequency >>  0) & 0xFF;  //LSB 
  loraSpi.transferSPI(5);  

  // Set PA parameters +22dBm
  loraSpi.spiBuff[0] = 0x95;
  loraSpi.spiBuff[1] = PA_DUTY_CYCLE;
  loraSpi.spiBuff[2] = PA_HP_MAX;
  loraSpi.spiBuff[3] = PA_DEVICE;
  loraSpi.spiBuff[4] = PA_LUT;
  loraSpi.transferSPI(5);
  
  // Set TX parameters
  loraSpi.spiBuff[0] = 0x8E;
  loraSpi.spiBuff[1] = TX_PARAMS_POWER;
  loraSpi.spiBuff[2] = TX_PARAMS_RAMP_TIME;
  loraSpi.transferSPI(3);

  // Set send/receive buffer base address
  loraSpi.spiBuff[0] = 0x8F;
  loraSpi.spiBuff[1] = 0x00;
  loraSpi.spiBuff[2] = 0x00;
  loraSpi.transferSPI(3);
  ...
  loraSpi.spiBuff[0] = 0x8B;                
  loraSpi.spiBuff[1] = SX1262_MODULATION_PARAMETERS_SPREADING_FACTOR;               
  loraSpi.spiBuff[2] = SX1262_MODULATION_PARAMETERS_BANDWIDTH;
  loraSpi.spiBuff[3] = SX1262_MODULATION_PARAMETERS_CODING_FACTOR;
  loraSpi.spiBuff[4] = SX1262_MODULATION_PARAMETERS_LOW_DATARATE_OPTIMIZE;
  loraSpi.transferSPI(5);

  // SetDioIrqParams
  loraSpi.spiBuff[0] = 0x08;
  loraSpi.spiBuff[1] = 0xFF;
  loraSpi.spiBuff[2] = 0xFF;
  loraSpi.spiBuff[3] = 0x00; 
  loraSpi.spiBuff[4] = 0x02; 
  loraSpi.spiBuff[5] = 0x00;
  loraSpi.spiBuff[6] = 0x01; 
  loraSpi.spiBuff[7] = 0x00;
  loraSpi.spiBuff[8] = 0x00;
...
void SX1262_Loracom::resetSX1262(){
  ...
  delay(10);
  waitUntilReady();
  if (DEBUG_LORA_PRINTOUTS) {Serial.println(F("SX1262 Reset"));}
  ...
  if (loraSpi.readRegister(0x0740) != 0x14 || loraSpi.readRegister(0x0741) != 0x24){
    if (DEBUG_LORA_ERROR) {Serial.println(F("SPI communication error"));}
    while (true);
  }
  else{
    if (DEBUG_LORA_PRINTOUTS) {Serial.println(F("SPI communication OK"));}
	;
  }
  return;
}
...
void SX1262_Loracom::radioTransmit(int dataLen){
  ...
  { 
    ...
    if (DEBUG_LORA_ERROR) {Serial.println(F("##ERROR## Transmit data length is longer than MAX_TRANSMIT_DATA_LENGTH. Truncated! "));}
  }
  if (dataLen > TRANSMIT_RECEIVE_BUFFER_SIZE) 
  { 
    dataLen = TRANSMIT_RECEIVE_BUFFER_SIZE;
    if (DEBUG_LORA_ERROR) {Serial.println(F("##ERROR## Transmit data length is longer than TRANSMIT_RECEIVE_BUFFER_SIZE. Truncated! "));}
   }

  if (DEBUG_LORA_MESSAGE)
  {
    Serial.print(F("Lora transmit: "));
  ...
  //SetPacketParameters
  loraSpi.spiBuff[0] = 0x8C;          
  loraSpi.spiBuff[1] = SX1262_PACKET_PARAMETERS_PREAMBLE_LENGTH_MSB;
  loraSpi.spiBuff[2] = SX1262_PACKET_PARAMETERS_PREAMBLE_LENGTH_MSB;
  loraSpi.spiBuff[3] = SX1262_PACKET_PARAMETERS_HEADER_TYPE;
  loraSpi.spiBuff[4] = dataLen; //Payload Length (Max is 255 bytes)
  loraSpi.spiBuff[5] = SX1262_PACKET_PARAMETERS_CRC_ON_OFF;
  loraSpi.spiBuff[6] = SX1262_PACKET_PARAMETERS_INVERT_IQ;
  loraSpi.transferSPI(7);
  
  //Transmit
  loraSpi.spiBuff[0] = 0x83;          
  loraSpi.spiBuff[1] = SX1262_TRANSMIT_TIMEOUT_MSB;
  loraSpi.spiBuff[2] = SX1262_TRANSMIT_TIMEOUT_MID;
  loraSpi.spiBuff[3] = SX1262_TRANSMIT_TIMEOUT_LSB;
  loraSpi.transferSPI(4);

  delay(10);
  uint32_t startTransmitTime = millis();
  if (DEBUG_LORA_PRINTOUTS) {Serial.println(F("Transmitting"));}
  ...
  if (digitalRead(SX1262_DIO2) == false){
      if (DEBUG_LORA_ERROR) {Serial.println(F("Transmit error timer expired"));}
    }
  getIrqStatus();
  //Tell the radio to clear the interrupt, and set the pin back inactive.
  loraSpi.spiBuff[0] = 0x02;          //Opcode for ClearIRQStatus command
  loraSpi.spiBuff[1] = 0xFF;          //IRQ bits to clear (MSB) (0xFFFF means clear all interrupts)
  loraSpi.spiBuff[2] = 0xFF;          //IRQ bits to clear (LSB)
  loraSpi.transferSPI(3);
  
  //Disable Transmitter
  rfSwitch(RF_SWITCH_OFF);
  if (DEBUG_LORA_PRINTOUTS)
  {
...
void SX1262_Loracom::setModeReceive() {
 ...
 // Set standby mode
  ...
  loraSpi.spiBuff[2] = SX1262_PACKET_PARAMETERS_PREAMBLE_LENGTH_MSB;
  loraSpi.spiBuff[3] = SX1262_PACKET_PARAMETERS_HEADER_TYPE;
  loraSpi.spiBuff[4] = 0xFF; //Payload Length (Max is 255 bytes)
  loraSpi.spiBuff[5] = SX1262_PACKET_PARAMETERS_CRC_ON_OFF;
  loraSpi.spiBuff[6] = SX1262_PACKET_PARAMETERS_INVERT_IQ;
  loraSpi.transferSPI(7);
  ...
  loraSpi.spiBuff[0] = 0x82;          //0x82 is the opcode for "SetRX"
  loraSpi.spiBuff[1] = SX1262_RECEIVE_TIMEOUT_MSB;
  loraSpi.spiBuff[2] = SX1262_RECEIVE_TIMEOUT_MID;
  loraSpi.spiBuff[3] = SX1262_RECEIVE_TIMEOUT_LSB;
  loraSpi.transferSPI(4);

  // Disable receive mode
  rfSwitch(RF_SWITCH_OFF);

  if (DEBUG_LORA_PRINTOUTS) {Serial.println(F("RX listening"));}
  return;
}
...
int SX1262_Loracom::receive_async() {
  ...
  if (getIrqStatus())
  {
    ...
    loraSpi.spiBuff[0] = 0x14;          //Opcode for get packet status
    loraSpi.spiBuff[1] = 0x00;          //Dummy byte. Returns status
    loraSpi.spiBuff[2] = 0x00;          //Dummy byte. Returns rssi
    loraSpi.spiBuff[3] = 0x00;          //Dummy byte. Returns snd
    loraSpi.spiBuff[4] = 0x00;          //Dummy byte. Returns signal RSSI
    loraSpi.transferSPI(5);

    // Check transmission data
    lastReceivedRSSI = (-((int)loraSpi.spiBuff[2]) / 2);
    lastReceivedSNR = (((int)loraSpi.spiBuff[3]) / 4);
    lastReceivedSignalRSSI = (-((int)loraSpi.spiBuff[4]) / 2);
    ...
    loraSpi.spiBuff[0] = 0x13;          //Opcode for GetRxBufferStatus command
    loraSpi.spiBuff[1] = 0x00;          //Dummy.  Returns radio status
    loraSpi.spiBuff[2] = 0x00;          //Dummy.  Returns loraPacketLength
    loraSpi.spiBuff[3] = 0x00;          //Dummy.  Returns memory offset (address)
    loraSpi.transferSPI(4);

    int payloadLen = loraSpi.spiBuff[2];
    int offset = loraSpi.spiBuff[3];
    ...
    if (payloadLen > TRANSMIT_RECEIVE_BUFFER_SIZE - 1){   //Leave room to add eol during receive buffer transfer
      ...
      if (DEBUG_LORA_ERROR)
      {
        Serial.print(F("ERROR! Received message length is longer than receive buffer. Message is truncated! Max message length is "));
...
bool SX1262_Loracom::getIrqStatus(){
  ...
  if ((loraSpi.spiBuff[3]) & 0x01) {
    if (DEBUG_LORA_PRINTOUTS){Serial.println(F("IRQ set: TxDone"));}
  }
  if ((loraSpi.spiBuff[3]) & 0x02) {
    if (DEBUG_LORA_PRINTOUTS){Serial.println(F("IRQ set: RxDone"));}
	;
  }
  if ((loraSpi.spiBuff[3]) & 0x04) {
    if (DEBUG_LORA_PRINTOUTS){Serial.println(F("IRQ set: Preamble detected"));}
	;
  }
  if ((loraSpi.spiBuff[3]) & 0x08) {
    if (DEBUG_LORA_PRINTOUTS){Serial.println(F("IRQ set: FSK Valid Sync Word detected"));}
	;
  }
  if ((loraSpi.spiBuff[3]) & 0x10) {
    if (DEBUG_LORA_PRINTOUTS){Serial.println(F("IRQ set: Valid LoRa Header received"));}
	;
  }
  if ((loraSpi.spiBuff[3]) & 0x20) {
    if (DEBUG_LORA_PRINTOUTS){Serial.println(F("IRQ Sync: LoRa Header CRC error"));}
    IrqOk = false;
  }
  if ((loraSpi.spiBuff[3]) & 0x40) {
    if (DEBUG_LORA_PRINTOUTS){Serial.println(F("IRQ Sync: Wrong CRC received"));}
    IrqOk = false;
  }
  if ((loraSpi.spiBuff[3]) & 0x80) {
    if (DEBUG_LORA_PRINTOUTS){Serial.println(F("IRQ Sync: Channel activity detection finished"));}
	;
  }
  if ((loraSpi.spiBuff[2]) & 0x01) {
    if (DEBUG_LORA_PRINTOUTS){Serial.println(F("IRQ set: Channel activity detected"));}
	;
  }
  if ((loraSpi.spiBuff[2]) & 0x02) {
    if (DEBUG_LORA_PRINTOUTS){Serial.println(F("IRQ set: Rx or Tx Timeout"));}
    IrqOk = false;
  }  
  return IrqOk;
}
...
errorsStruct SX1262_Loracom::getDeviceErrors(){
  ...
  //Parse out the status (see datasheet for what each bit means)
  if ((loraSpi.spiBuff[3]) & 0x01) {
    if (DEBUG_LORA_ERROR) {Serial.println(F("##ERROR## Device error: RC64K_CALIB_ERR"));}
	deviceErrors.rc64kCalibrationError = true;
    deviceErrors.allOk = false;
  }
  if ((loraSpi.spiBuff[3]) & 0x02) {
    if (DEBUG_LORA_ERROR) {Serial.println(F("##ERROR## Device error:  RC13M_CALIB_ERR"));}
	deviceErrors.rc13mCalibrationError = true;
    deviceErrors.allOk = false;
  }
  if ((loraSpi.spiBuff[3]) & 0x04) {
    if (DEBUG_LORA_ERROR) {Serial.println(F("##ERROR## Device error: PLL_CALIB_ERR"));}
	deviceErrors.pllCalibrationError = true;
    deviceErrors.allOk = false;
  }
  if ((loraSpi.spiBuff[3]) & 0x08) {
    if (DEBUG_LORA_ERROR) {Serial.println(F("##ERROR## Device error: ADC_CALIB_ERR"));}
	deviceErrors.adcCalibrationError = true;
    deviceErrors.allOk = false;
  }
  if ((loraSpi.spiBuff[3]) & 0x10) {
    if (DEBUG_LORA_ERROR) {Serial.println(F("##ERROR## Device error: IMG_CALIB_ERR"));}
	deviceErrors.imgCalibrationError = true;
    deviceErrors.allOk = false;
  }
  if ((loraSpi.spiBuff[3]) & 0x20) {
    if (DEBUG_LORA_ERROR) {Serial.println(F("##ERROR## Device error: XOSC_START_ERR"));}
	deviceErrors.xoscStartError = true;
    deviceErrors.allOk = false;
  }
  if ((loraSpi.spiBuff[3]) & 0x40) {
    if (DEBUG_LORA_ERROR) {Serial.println(F("##ERROR## Device error: PLL_LOCK_ERR"));}
	deviceErrors.pllLockError = true;
    deviceErrors.allOk = false;
  }
  if ((loraSpi.spiBuff[2]) & 0x01) {
    if (DEBUG_LORA_ERROR) {Serial.println(F("##ERROR## Device error: PA_RAMP_ERR"));}
	deviceErrors.paRampError = true;
    deviceErrors.allOk = false;
  }
  if (deviceErrors.allOk){
    if (DEBUG_LORA_PRINTOUTS) {Serial.println(F("Device OK"));}
  }
  else{
    // Clear device errors
    loraSpi.spiBuff[0] = 0x07;          
...
commandStatusStruct SX1262_Loracom::getCommandStatus(){
  ...
  loraSpi.spiBuff[0] = 0xC0;          
  loraSpi.spiBuff[1] = 0x00;               
  loraSpi.transferSPI(2);

  uint8_t cmdStatus = (loraSpi.spiBuff[1] >> 1) & 0x7;//Command status is bits [3:1] (3-bits)
  
  commandStatusStruct commandStatus = {false, false, false, false, false};

  //Parse out the status (see datasheet for what each bit means)
  if ((cmdStatus) == 0x02) {
    if (DEBUG_LORA_PRINTOUTS) {Serial.println(F("Command status:  A packet has been successfully received and data can be retrieved"));}
	commandStatus.rxSuccessPacketReady = true;
  }
  if ((cmdStatus) == 0x03) {
    if (DEBUG_LORA_ERROR) {Serial.println(F("Command status: A transaction from host took too long to complete and triggered an internal watchdog"));}
	commandStatus.internalWatchdogTripped = true;
  }
  if ((cmdStatus) == 0x04) {
    if (DEBUG_LORA_ERROR) {Serial.println(F("Command status: . Processor was unable to process command either because of an invalid opcode or because an incorrect number of parameters has been provided"));}
	commandStatus.invalidOpCodeOrWrongNumberOfParameters = true;
  }
  if ((cmdStatus) == 0x05) {
    if (DEBUG_LORA_ERROR) {Serial.println(F("Command status: The command was successfully processed, however the chip could not execute the command; for instance it was unable to enter the specified device mode or send the requested data"));}
	commandStatus.couldNotExecuteCommand = true;
  }
  if ((cmdStatus) == 0x06) {
    if (DEBUG_LORA_ERROR) {Serial.println(F("Command status: The transmission of the current packet has terminated"));}
...
chipModeStruct SX1262_Loracom::getChipMode(){
  ...
  uint8_t chipM = (loraSpi.spiBuff[1] >> 4) & 0x7;     //Chip mode is bits [6:4] (3-bits)
  
  chipModeStruct chipMode = {false, false, false, false, false};

  //Parse out the status (see datasheet for what each bit means)
  if ((chipM) == 0x02) {
    if (DEBUG_LORA_PRINTOUTS) {Serial.println(F("Device status: STBY_RC"));}
	chipMode.stbyRC = true;
  }
  if ((chipM) == 0x03) {
    if (DEBUG_LORA_PRINTOUTS) {Serial.println(F("Device status: STBY_XOSC"));}
	chipMode.stbyXosc = true;
  }
  if ((chipM) == 0x04) {
    if (DEBUG_LORA_PRINTOUTS) {Serial.println(F("Device status: FS"));}
	chipMode.Fs = true;
  }
  if ((chipM) == 0x05) {
    if (DEBUG_LORA_PRINTOUTS) {Serial.println(F("Device status: RX"));}
	chipMode.Rx = true;
  }
  if ((chipM) == 0x06) {
    if (DEBUG_LORA_PRINTOUTS) {Serial.println(F("Device status: TX"));}
	chipMode.Tx = true;
  }
  return chipMode;
}
